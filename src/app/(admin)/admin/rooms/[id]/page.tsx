...
export default function EditRoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Find room category parameters
  const existingRoom = dummyRooms.find(r => r.id === id) || dummyRooms[0];

  // Form states initialized with existing parameters
  const [title, setTitle] = useState(existingRoom.title);
  const [slug, setSlug] = useState(existingRoom.slug);
  const [type, setType] = useState<RoomType>(existingRoom.type);
  const [basePrice, setBasePrice] = useState(existingRoom.basePrice);
  const [extraGuestPrice, setExtraGuestPrice] = useState(existingRoom.extraGuestPrice);
  const [minCapacity, setMinCapacity] = useState(existingRoom.capacity.min);
  const [maxCapacity, setMaxCapacity] = useState(existingRoom.capacity.max);
  const [description, setDescription] = useState(existingRoom.description);
  const [amenities, setAmenities] = useState<string[]>(existingRoom.amenities);
  const [images, setImages] = useState<{ url: string; publicId: string }[]>(existingRoom.images);
  const [isActive, setIsActive] = useState(existingRoom.isActive);
  const [isFeatured, setIsFeatured] = useState(true); // default featured
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    alert("Room listing parameters saved successfully (local state bypass)!");
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6 max-w-5xl mx-auto pb-16">
      <div className="flex items-center gap-3">
        ...
        <PageHeader 
          title={`Edit Room: ${existingRoom.title}`} 
          description="Modify listings description text, prices, and upload replacement pictures."
        />
      </div>
      ...
      <div className="space-y-1">
          ...
          <div className="grid gap-2.5 grid-cols-2 sm:grid-cols-3">
            {availableAmenities.map(item => (
              ...
              >
                ...
                />
                <span className="text-slate-755 dark:text-zinc-350">{item}</span>
              </label>
            ))}
          </div>
        </FormSection>
        ...
        <FormSection 
          ...
          description="Set visibility filters and listing status."
        >
